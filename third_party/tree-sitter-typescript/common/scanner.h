...
static bool scan_type_query_import_start(TSLexer *lexer) {
    static const char type_query_keyword[] = "typeof";
    static const char import_keyword[] = "import";

    lexer->result_symbol = TYPE_QUERY_IMPORT_START;
    for (unsigned i = 0; i < sizeof(type_query_keyword) - 1; i++) {
        if (lexer->lookahead != type_query_keyword[i]) {
            return false;
        }
        advance(lexer);
    }
    ...
    for (unsigned i = 0; i < sizeof(import_keyword) - 1; i++) {
        if (lexer->lookahead != import_keyword[i]) {
            return false;
        }
        skip(lexer);
    }
...
static inline bool external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    if (valid_symbols[TYPE_QUERY_IMPORT_START] && lexer->lookahead == 't') {
        return scan_type_query_import_start(lexer);
    }
