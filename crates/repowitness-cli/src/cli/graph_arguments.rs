...
impl GraphWorkspaceArguments {
    ...
    fn into_context(self) -> Result<GraphWorkspaceContext, &'static str> {
        match (
            ...
            (Some(_), _, _) => Err(
                "error: graph --repository-id cannot be combined with connected workspace selectors\n",
            ),
            (None, None, None) => Err(
                "error: graph requires --repository-id or connected workspace selectors; use graph --help\n",
            ),
            (None, None, Some(_)) => {
                Err("error: graph --source-slot-id requires --connected-workspace-id\n")
            }
            (None, Some(_), None) => {
                Err("error: graph --connected-workspace-id requires --source-slot-id\n")
            }
        }
    }
}
