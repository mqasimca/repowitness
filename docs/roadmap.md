...
| Context compiler and read tools | Implemented | The canonical `context-build` CLI/MCP path uses deterministic evidence-balanced allocation with conservative byte-budget admission, exact source expansion, current-memory exclusion rules, graph/SCIP/history eligibility, explicit coverage, omissions, limits, cancellation, and source-only fallback; `memory_recall` and transactionally pinned raw/recognized parser diagnostics remain available. |
...
and [ADR-0036](adr/0036-phase2-context-ranking-profiles.md), which defines a
separately versioned evidence-ranking path. Proposed
[ADR-0055](adr/0055-canonical-evidence-balanced-context-command.md) promotes
the evaluated path through one canonical context command. Accepted
...
- Initial progress: `evidence-balanced-v1` is the canonical `context-build`
  CLI/MCP contract. It has
